...
-- (priority, then FIFO) so the batch claim loop keeps the same fairness, and it
-- is backed by the same partial index idx_agent_task_queue_claim_candidates
-- (= ANY on the indexed runtime_id column is served by the index).
SELECT * FROM agent_task_queue
