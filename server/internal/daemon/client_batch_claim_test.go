...
// batch claim (MUL-4257): the client POSTs to /api/daemon/claim with the full
...
func TestClient_ClaimTasks_PostsRuntimeSetAndParsesTasks(t *testing.T) {
	...
	tasks, err := c.ClaimTasks(context.Background(), []string{"rt-a", "rt-b", "rt-c"}, 3)
	if err != nil {
		t.Fatalf("ClaimTasks: %v", err)
	}

	if gotPath != "/api/daemon/claim" {
		t.Errorf("path = %q, want /api/daemon/claim", gotPath)
	}
...
func TestClient_ClaimTasks_EmptyResult(t *testing.T) {
	...
	tasks, err := c.ClaimTasks(context.Background(), []string{"rt-a"}, 1)
	if err != nil {
		t.Fatalf("ClaimTasks: %v", err)
	}
	if len(tasks) != 0 {
