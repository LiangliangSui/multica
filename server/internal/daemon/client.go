...
// every runtime the daemon hosts. Each returned Task carries its own RuntimeID
// so the daemon routes it to the matching runtime locally. The request runs
// under a short, request-scoped deadline (batchClaimRequestTimeout) rather than
// the shared 30s control-plane timeout so one slow claim cannot stall the whole
// batch; the deadline propagates to the server and cancels the in-flight query
// there too.
func (c *Client) ClaimTasks(ctx context.Context, runtimeIDs []string, maxTasks int) ([]*Task, error) {
	reqCtx, cancel := context.WithTimeout(ctx, batchClaimRequestTimeout)
	defer cancel()
	var resp struct {
		Tasks []*Task `json:"tasks"`
	}
	if err := c.postJSON(reqCtx, "/api/daemon/claim", map[string]any{
